...
use crate::models::request::elements::model_note::DataElementNote;
...
#[post(
    "/element/note",
    data = "<option_data_element_note_model>",
    format = "json"
)]
pub async fn post_element_note(
    auth: AuthorizedUser,
    option_data_element_note_model: Option<Json<DataElementNote>>,
    database: &State<MongoDB>,
) -> Result<(Status, Json<Element>), (Status, Json<ErrorResponse>)> {
    match option_data_element_note_model {
        None => Err(WRONG_REQUEST),
        Some(element_note) => {
            let element = Element {
                id_user: auth.user_id,
                name: element_note.name.clone(),
                login: None,
                password: None,
                url: None,
                owners_name: None,
                ...
                index: None,
                country: None,
                description: element_note.description.clone(),
                favorite: element_note.favorite,
            };
