...
use crate::constants::{
    LEN_DESCRIPTION_IN_ELEMENT_LOGIN, LEN_LOGIN_IN_ELEMENT_LOGIN, LEN_PASSWORD_IN_ELEMENT_LOGIN,
    WEAK_LOGIN, WEAK_PASSWORD, WRONG_DESCRIPTION, WRONG_REQUEST,
};
use crate::database::connect_to_db::MongoDB;
use crate::helper::{get_valid_desc, object_id_parse_str, ValidDescAndUrlError};
...
fn check_data_element_login(
    option_element: Option<Json<DataElementLogin>>,
) -> DataElementLoginError {
    match option_element {
        None => DataElementLoginError::NoneElement,
        Some(element) => match valid_data_element_login(element) {
            ValidDataElementLoginError::Ok(element_is_valid) => {
                DataElementLoginError::Ok(element_is_valid)
            }
...
fn valid_data_element_login(element: Json<DataElementLogin>) -> ValidDataElementLoginError {
    match get_valid_login_and_password(
        ...
        TypeValidTwoStr::Ok => {
            match get_valid_desc(
                element.description.clone(),
                LEN_DESCRIPTION_IN_ELEMENT_LOGIN,
            ) {
                ValidDescAndUrlError::Ok => ValidDataElementLoginError::Ok(element),
                ValidDescAndUrlError::DescriptionIsNotValid => {
                    ValidDataElementLoginError::BadDescription
                }
