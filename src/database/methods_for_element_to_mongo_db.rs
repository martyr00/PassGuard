...
impl MongoDB {
    pub async fn post_element(&self, element:String, preview:String, user_id: String) -> mongodb::error::Result<()> {
    ...
    pub async fn get_previews(&self, user_id:String) -> mongodb::error::Result<Vec<Preview>> {
        ...
        let mut cursor = collection_element_preview.find(bson::doc! { "user_id": user_id }, None).await?;
    ...
    pub async fn get_element(&self, element_id:String) -> mongodb::error::Result<Option<Element>> {
        ...
        let element = collection_element.find_one(bson::doc! { "_id": element_id }, None).await?;
